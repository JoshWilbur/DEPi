...
long generation = 0;
int alive = 0;
int perf_rules[N_STATS] = {0};
int perf_render[N_STATS] = {0};
int perf_loop[N_STATS] = {0};
...
void Render(int *board, int h, int w, int speed_us, int time_rules, int time_render, int time_loop, int stats_flag){
    ...
    if (stats_flag){
        // Get average runtime for rules and render functions
        float avg_rule = 0; 
        float avg_render = 0;
        float avg_loop = 0;
        ...
        // Print stats
        mvprintw(h+2, 10, "Generation: %ld | Population: %d | Speed: %.1fms | Press 'q' to quit.", generation, alive, speed_ms);
        mvprintw(h+3, 10, "LIVE: Rule time: %dus | Render time: %dus | Loop time: %dus", time_rules, time_render, time_loop);
        mvprintw(h+4, 10, "AVERAGE: Rule time: %.1fus | Render time: %.1fus | Loop time: %.1fus", avg_rule, avg_render, avg_loop);
    }
